...
class Attributes(unittest.TestCase):
    def test_should_not_be_nullifiable_by_default(self):
        with self.assertRaises(ValueError):
            Attribute(str)(None)

    def test_should_be_nullifiable_if_specified(self):
    ...
    def test_should_use_fallback_if_specified(self):
    ...
    def test_should_call_fallback_if_function(self):
        def test_function():
            return 'test'
    ...
    def test_should_cast_value_to_given_type(self):
        try:
            uut = Attribute(str, fallback='test')
    ...
    def test_should_allow_named_attributes(self):
        uut = Attribute(str, name='@foobar')
        self.assertEqual(uut('def').name, '@foobar')

    def test_should_raise_when_type_is_none(self):
        with self.assertRaises(ValueError):
            Attribute(None)
